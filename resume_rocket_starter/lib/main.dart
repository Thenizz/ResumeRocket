...
// Features:
// - Local resume editor (save locally using shared_preferences)
// - Template selection (ATS / Visual)
// - PDF generation & share using `pdf` + `printing`
// - Simple offline "AI rewrite" mock (no external APIs)
// - "Admin" toggle to enable premium features (no payments required)
//
// NOTE: This is a starter scaffold meant to be expanded. See README.md for build/install steps.
import 'dart:convert';
...
class _EditorScreenState extends State<EditorScreen> {
  ...
  void _aiRewriteSummary() {
    // Simple offline "AI" mock: rewrite by prefixing and cleaning length.
  ...
  String _mockRewrite(String input) {
    ...
    // A tiny heuristic: split sentences and make short bullet-style summary
  ...
  void _selectTemplate() async {
    final choice = await showModalBottomSheet<String>(context: context, builder: (_) {
      return SafeArea(child: Column(
        ...
        children: [
          ...
          ListTile(
            title: Text('Creative Visual (premium)'),
            subtitle: Text('Requires premium'),
            onTap: () => Navigator.pop(context, 'creative_visual'),
          ),
          SizedBox(height:16),
        ],
      ));
    ...
    if (choice != null) {
      // If premium templates are chosen but premium not enabled, block
  ...
  Future<Uint8List> _generatePdfBytes(Map<String, dynamic> resume) async {
    ...
    // Two simple templates: ATS Classic and Modern Clean; premium Creative Visual will be similar but decorative.
    if (template == 'ats_classic') {
      ...
      // modern_clean and creative_visual share formatting but with small differences
      pdf.addPage(pw.MultiPage(pageFormat: PdfPageFormat.a4, build: (pw.Context ctx) {
        return [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Editor — ${(r['title'] ?? 'Untitled')}'),
        actions: [
      ...
      body: Form(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              TextFormField(
                initialValue: r['title'] ?? 'Untitled Resume',
                decoration: InputDecoration(labelText: 'Resume Title'),
                onChanged: (v) => r['title'] = v,
              ),
              SizedBox(height:8),
              TextFormField(
                initialValue: r['name'] ?? '',
              ...
              SizedBox(height:8),
              TextFormField(
                initialValue: r['phone'] ?? '',
                decoration: InputDecoration(labelText: 'Phone'),
                onChanged: (v) => r['phone'] = v,
              ),
              SizedBox(height:8),
              TextFormField(
                initialValue: r['summary'] ?? '',
                decoration: InputDecoration(labelText: 'Professional summary'),
                maxLines: 4,
                onChanged: (v) => r['summary'] = v,
              ),
              SizedBox(height:6),
              Row(children: [
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        TextFormField(
                          initialValue: e['role'],
                          decoration: InputDecoration(labelText: 'Role / Title'),
                        ...
                        TextFormField(
                          ...
                          decoration: InputDecoration(labelText: 'Years (e.g. 2020-2023)'),
                          onChanged: (v) => e['years'] = v,
                        ),
                        TextFormField(
                          initialValue: e['desc'],
                          decoration: InputDecoration(labelText: 'Description / achievements'),
                          maxLines: 3,
                          onChanged: (v) => e['desc'] = v,
                        ),
                    ...
                    child: Column(
                      children: [
                        TextFormField(
                          ...
                          decoration: InputDecoration(labelText: 'Degree / Qualification'),
              ...
              Align(alignment: Alignment.centerLeft, child: TextButton.icon(onPressed: () {
                setState(() {
                  (r['education'] as List).add({'school':'','degree':'','years':''});
                });
              }, icon: Icon(Icons.add), label: Text('Add education'))),
              SizedBox(height:8),
              TextFormField(
                initialValue: r['skills'] ?? '',
                decoration: InputDecoration(labelText: 'Skills (comma separated)'),
                onChanged: (v) => r['skills'] = v,
              ),
              SizedBox(height:16),
